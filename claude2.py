...
class StockAnalyzer:
    ...
    def analyze_portfolio(self, selected_stocks):
        ...
        if not selected_stocks:  # Add check for empty selection
    ...
    def run(self):
        ...
        with col1:
            # Get user input using multiselect
            temp_selection = st.multiselect(
                "Select stocks for your portfolio:",
                options=self.df['name'],
                default=st.session_state.selected_stocks,
                key='temp_stocks'  # Add a unique key
            )

        with col2:
            # Add analyze button
            if st.button("Analyze Portfolio"):
                st.session_state.selected_stocks = temp_selection
                st.session_state.submit_clicked = True

        with col3:
            # Add refresh button for new period
            if st.button("New Random Period"):
                st.session_state.submit_clicked = True  # Ensure analysis runs with new period
        ...
        elif not temp_selection:  # Changed from st.session_state.selected_stocks
