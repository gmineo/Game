...
if stock_input:
    ...
    # Aggiungi il primo frame manualmente
    ...
    fig.update_layout(
        ...
        sliders=[{
            "steps": [{"args": [[str(k)], {"frame": {"duration": 100, "redraw": True}, "mode": "immediate"}],
                       "label": str(portfolio_df['Date'].iloc[k]),
                       "method": "animate"} for k in range(len(portfolio_df))]
        }]
    )

    fig.frames = frames

    # Mostra il grafico animato in Streamlit
    st.plotly_chart(fig)
else:
    st.write("Seleziona almeno un titolo per iniziare.")
